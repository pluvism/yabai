...
test('should validate with zod schema', async () => {
    const bot = new Yabai()
    bot.cmd('sum :a :b', z.object({ a: z.coerce.number(), b: z.coerce.number() }), async ({ params, msg }) => {
...
test('should handle a predicate with hears()', async () => {
    ...
    await bot.handle({
        body: 'hello there',
        raw: { message: { conversation: 'hello there' }, key: { remoteJid: 'test' } } as any,
    })
    ...
    await bot.handle({
        body: 'goodbye there',
        raw: { message: { conversation: 'goodbye there' }, key: { remoteJid: 'test' } } as any,
    })
...
test('should generate help message', async () => {
    ...
    bot.cmd('ping', ({ msg }) => msg.reply('pong'), { description: 'A simple ping command' })
    ...
    assert.ok(repliedText.includes('*Available Commands:*'), 'Help message should have a title')
    assert.ok(repliedText.includes('ping: A simple ping command'), 'Help message should include ping command')
    assert.ok(repliedText.includes('foo: No description'), 'Help message should include foo command with default description')
    assert.ok(repliedText.includes('help: Displays this help message'), 'Help message should include itself')
})
